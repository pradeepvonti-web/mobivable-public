        ...
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <div className="space-y-3">
            ...
            {Object.entries(data.planCounts).map(([plan, count]) => {
              ...
              const color = plan === "pro" ? "bg-muted-foreground" : plan === "starter" ? "bg-muted-foreground" : "bg-muted-foreground";
              return (
                ...
                <tr key={s.id} className="hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-mono uppercase px-2 py-0.5 rounded-full ${
                      s.status === "active" || s.status === "trialing" ? "bg-primary/15 text-primary" :
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-mono uppercase px-2 py-0.5 rounded-full ${
                      s.environment === "live" ? "bg-muted/15 text-foreground" : "bg-muted/15 text-foreground"
                  ...
                  <td className="px-4 py-3 text-right">
                    {s.status !== "canceled" && (
                      <button
                        ...
                        disabled={busy === s.id}
                        className="text-xs px-2.5 py-1 rounded-lg border border-destructive/30 text-destructive hover:bg-destructive/10 disabled:opacity-50"
                      >
                        {busy === s.id ? "…" : "Cancel"}
                      </button>
                    )}
