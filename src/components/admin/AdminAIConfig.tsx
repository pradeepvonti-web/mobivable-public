...
export function AdminAIConfig() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display uppercase text-2xl tracking-tight">AI Provider Configuration</h2>
        <p className="text-sm text-muted-foreground mt-1">{configured}/5 providers configured · Active: {activeProvider}</p>
      </div>

      {/* Status banner */}
      <div className={`flex items-center gap-3 p-5 rounded-lg border ${
        configured > 0 ? "bg-primary/10 border-primary/30" : "bg-destructive/10 border-destructive/30"
      }`}>
        {configured > 0 ? <Check className="h-5 w-5 text-primary" /> : <X className="h-5 w-5 text-destructive" />}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {providers.map((p) => {
          ...
          if (!meta) return null;
          return (
            <div key={p.id} className={`rounded-lg border p-5 space-y-4 transition-all ${
              p.isActive ? "border-primary/40 bg-primary/5 ring-1 ring-primary/20" : p.configured ? "border-primary/30 bg-card" : "border-border bg-card/40"
            }`}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {p.configured && !p.isActive && <span className="text-[8px] font-mono uppercase bg-primary/20 text-primary px-1.5 py-0.5 rounded-full">ready</span>}
                  </div>
                  {showKeys && (
                    <code className="text-[9px] text-muted-foreground font-mono">{meta.envKey}={p.configured ? "••••••••" : "(not set)"}</code>
                  )}
                </div>
                {p.configured ? <Check className="h-4 w-4 text-primary" /> : (
      ...
      <div className="rounded-lg border border-primary/20 bg-primary/5 p-6 space-y-4">
        ...
        <pre className="text-[10px] font-mono bg-card border border-border rounded-lg p-4 overflow-x-auto text-foreground/60">
