 ...
 *
 * Keep this lean and tsc-clean: it is the floor, not the app. Expo SDK 54 /
 * expo-router 6 (RN 0.81 / React 19.1), file-based routing under `app/`. Keep
 * the SDK in lockstep with the Expo Go app — it only runs the latest SDK.
 */
    ...
    "package.json": JSON.stringify(
      {
        ...
        dependencies: {
          // Expo SDK 54 — keep in lockstep with the Expo Go app (it only runs
          // the latest SDK, so an older SDK errors on device). RN 0.81 / React 19.1.
          expo: "~54.0.0",
          "expo-router": "~6.0.24",
          ...
          // SDK 54-pinned versions.
          "react-native-svg": "15.12.1", // charts/icons/QR via SVG primitives
          "expo-linear-gradient": "~15.0.8", // gradient hero/card surfaces
          ...
          "@react-native-async-storage/async-storage": "2.2.0", // Supabase session store on device
