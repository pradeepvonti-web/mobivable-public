...
import { renderSchemaToRn } from "./rn-renderer";
...
`,
    ...
    // app/index.tsx — schema-rendered first screen. The renderer is the
    // single source of truth shared with the Snack preview, so the
    // exported app paints the same widgets the user sees in the
    // browser. expo-router picks up the default export regardless of
    // the function name.
    const parsedSchema = parseAppSchema(project.result ?? "");
    const firstScreen = parsedSchema?.screens?.[0] ?? {
      id: "home",
      title: "Home",
      elements: [] as unknown[],
    };
    const renderedHome = renderSchemaToRn({
      appName,
      theme: {
        primary,
        background: bg,
      ...
      screen: firstScreen as { id?: string; title?: string; elements?: unknown[] },
    });
    // Merge any extra deps the renderer declared (e.g. react-native-svg
    // when we add chart widgets). For v1 it's only safe-area-context,
    // which the export already pinned above.
    Object.assign(dependencies, renderedHome.dependencies);
    zip.file("app/index.tsx", renderedHome.appTsx);
